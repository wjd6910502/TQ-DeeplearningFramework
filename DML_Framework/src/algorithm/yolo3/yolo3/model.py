...
from algorithm.yolo3.yolo3.utils import compose
...
def yolo_head(feats, anchors, num_classes, input_shape, calc_loss=False):
    ...
    grid_y = tf.tile(K.reshape(tf.range(grid_shape[0]), [-1, 1, 1, 1]),
        [1, grid_shape[1], 1, 1])
    grid_x = tf.tile(tf.reshape(tf.range(grid_shape[1]), [1, -1, 1, 1]),
        [grid_shape[0], 1, 1, 1])
    # grid = concatenate([grid_x, grid_y])
...
def yolo_correct_boxes(box_xy, box_wh, input_shape, image_shape):
    ...
    # boxes *= concatenate([image_shape, image_shape])
    boxes *= tf.concat([image_shape, image_shape], axis=-1)
    return boxes

def my_yolo_boxes_and_scores(feats, anchors, num_classes, input_shape, image_shape):
    ...
    box_confidence = tf.reshape(box_confidence, [-1])
...
def yolo_eval(yolo_outputs,
    ...
    # box_scores = concatenate(box_scores, axis=0)
    ...
    # boxes_ = concatenate(boxes_, axis=0)
    # scores_ = concatenate(scores_, axis=0)
    # classes_ = concatenate(classes_, axis=0)
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, print_loss=False):
    ...
    for l in range(num_layers):
        ...
        # pred_box = concatenate([pred_xy, pred_wh])
        pred_box = tf.concat([pred_xy, pred_wh], axis=-1)

        # Darknet raw box to calculate loss.
        raw_true_xy = y_true[l][..., :2]*grid_shapes[l][::-1] - grid
        # raw_true_wh = tf.log(y_true[l][..., 2:4] / anchors[anchor_mask[l]] * input_shape[::-1])  # modified
        raw_true_wh = tf.math.log(K.clip(y_true[l][..., 2:4] / anchors[anchor_mask[l]] * input_shape[::-1], 1e-8, 1.0))
