...
import (
	...
	// This Service
	pb "server/proto"
	"server/service/tq-service/handlers"
...
func NewEndpoints() svc.Endpoints {
	...
	// Endpoint domain.
	var (
		createtaskEndpoint    = svc.MakeCreateTaskEndpoint(service)
		gettaskstatusEndpoint = svc.MakeGetTaskStatusEndpoint(service)
	)
...
func Run(cfg Config) {
	...
	// Debug listener.
	go func() {
		util.Infof("transport", "debug", "addr", cfg.DebugAddr)
	...
	// HTTP transport.
	go func() {
		util.Infof("transport", "HTTP", "addr", cfg.HTTPAddr)
	...
	// gRPC transport.
	go func() {
		util.Infof("transport", "gRPC", "addr", cfg.GRPCAddr)
		...
		s := grpc.NewServer()
		pb.RegisterTqServer(s, srv)

		errc <- s.Serve(ln)
	}()

	// Run!
	util.Infof("exit", <-errc)
}
